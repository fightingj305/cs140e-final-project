...
void I2S_Init(I2S *i2s) {
    ...
    PUT32(PCM_MODE_A, ((i2s->frame_size * 2) << PCM_MODE_FLEN_BIT) | (i2s->frame_size << PCM_MODE_FSLEN_BIT));
    ...
    if (i2s->bclk.pin_num == 18 && i2s->fs.pin_num == 19 && i2s->din.pin_num == 20 && i2s->dout.pin_num == 21) {
        GPIO_Config(i2s->bclk.pin_num, AF0);
        GPIO_Config(i2s->fs.pin_num, AF0);
        GPIO_Config(i2s->din.pin_num, AF0);
        GPIO_Config(i2s->dout.pin_num, AF0);
    }
    else if (i2s->bclk.pin_num == 28 && i2s->fs.pin_num == 29 && i2s->din.pin_num == 30 && i2s->dout.pin_num == 31) {
        GPIO_Config(i2s->bclk.pin_num, AF2);
        GPIO_Config(i2s->fs.pin_num, AF2);
        GPIO_Config(i2s->din.pin_num, AF2);
        GPIO_Config(i2s->dout.pin_num, AF2);
    }
    else {
...
void I2S_Wait_2() {
    ...
    while ((GET32(PCM_CS_A) & PCM_CS_SYNC) != current_value ^ PCM_CS_SYNC);
}
