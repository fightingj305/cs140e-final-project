...
static const float log_table[LOG_TABLE_SIZE] = { // logarithmic scale from 200 hz to 15 kHz
    200.0f,    // t=0.000
    280.0f,    // t=0.067
    393.0f,    // t=0.133
    550.0f,    // t=0.200
...
float ads_to_cutoff_log(uint16_t ads_val) { // gives a logarithmic scale for the cutoff frequency
...
void set_cutoff(uint16_t ads_val) {
    ...
    float alpha_max = log_table[LOG_TABLE_SIZE - 1] / (log_table[LOG_TABLE_SIZE - 1] + (float)SAMPLING_RATE / (2.0f * 3.14159f));
    float gain = 0.75f * alpha_max / (alpha > 0.001f ? alpha : 0.001f); // do a slight overall gain to amend for the attenuation at low frequencies
